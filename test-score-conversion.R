...
colnames(students_ftft) <- c("puid", "citizenship_desc", "profile_academic_period", 
...
(nrow(SAT_new)+nrow(ACT_all)+nrow(no_test_all))==nrow(students_ftft)
students_ftft_equated <- rbind(SAT_new, ACT_all, no_test_all)
...
cor.test(students_ftft_equated$grad_1_overall_gpa, students_ftft_equated$SAT_new)
data.gpa <- students_ftft_equated %>% 
